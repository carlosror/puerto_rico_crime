...
shinyServer(function(input, output, session) {
    ...
    filter_data <- reactive({
        input$goButton
        calls <- read.csv("crimes.csv", encoding="UTF-8")
    ...
    output$DataTable <- renderDataTable({
        # Fetch relevant data
        relevant_data <- filter_data()
        relevant_data <- rename(relevant_data, replace=c("days" = "Días"))
        relevant_data <- rename(relevant_data, replace=c("periods" = "Períodos"))
        relevant_data <- rename(relevant_data, replace=c("categories" = "Crímenes"))
        # Use relevant columns for table
        relevant_data_table <- relevant_data[c("Fecha", "Días", "Períodos", "Crímenes")]
        relevant_data_table
    ...
    output$barplots <- renderPlot({
        ...
        crime_barplot <- ggplot(data = relevant_data) + ggtitle(paste("Crímenes en", isolate(input$year), "en alrededores de\n", isolate(input$address))) # basic plot + title
    ...
    output$debug <- renderPrint({
        ...
        # year
        crimes_year <- isolate(input$year)
        c(crimes_set, crimes_days, crimes_periods)
        unique(calls$categories)
    })
